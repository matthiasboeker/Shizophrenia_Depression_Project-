...
class result_en():
    ...
    def __init__(self, name, group, variant, trans_mat = 0, start_prob = 0, means = 0, cov=1.0, link1=1.0, link2=1.0 ):
        self.name = name 
        self.group = group
...
def load_entities(directory='/Users/matthiasboeker/Desktop/Master_Thesis/Schizophrenia_Depression_Project/Results/With_covariate/'): 
    ...
    for i in range(0,len(files),2):
        ...
        con.cov = res['covariance']
        con.start_prob = res[' start_prob']
        con.means = res[' means']
    ...
    for i in range(0,len(files),2):
        ...
        res = pd.read_csv(files[i], delimiter = ',')
        pat.cov = res['covariance']
        pat.start_prob = res[' start_prob']
        pat.means = res[' means']
